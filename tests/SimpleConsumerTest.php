...
class SimpleConsumerTest
{
    ...
    /**
     * Tests subscribe method chaining (returns $this).
     * Uses reflection to set expressions and verify the subscription map update.
     */
    public function testSubscribeReturnsThis()
    {
        ...
        // Simulate running state and pre-populate route data to avoid network call
        $refRunning = new \ReflectionProperty($consumer, 'isRunning');
        $refRunning->setAccessible(true);
        $refRunning->setValue($consumer, true);

        // Pre-populate route cache so getRouteData won't be called
        $refRoute = new \ReflectionProperty($consumer, 'subscriptionRouteDataCache');
        $refRoute->setAccessible(true);
        $refRoute->setValue($consumer, ['topic-2' => 'cached']);

        $result = $consumer->subscribe('topic-2', 'tagA');
        ...
        TestRunner::assertEqualsWithMessage(
            'tagA',
            $expressions['topic-2'],
            "topic-2 expression should be tagA"
        );
    }
